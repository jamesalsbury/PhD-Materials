...
lines(N1vec, predict(lo1), lty=3, col="blue")
...
lines(N1vec, predict(lo2), lty=4, col="red")
...
lines(N1vec, predict(lo3), lty=5, col="green")
...
lines(N1vec, predict(DesignAssuranceFuncNonInform()), lty=6, col="yellow")
...
```{r interim, echo=F}
 ...
 controlevents <- rep(NA, 1e4)
 treatmentevents <- rep(NA, 1e4)
 for (i in 1:1e4){
  controlevents[i] <- fit@sim$samples[[1]][[4]][i]
  treatmentsevents[i] <- fit@sim$samples[[1]][[5]][i]
