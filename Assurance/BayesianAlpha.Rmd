...
```
We can use the information above to calculate the sample size required to find a statistically significance difference between these two groups. We wish to run our trial at $\alpha$ = 0.05 and power $1-\beta$ = 0.9. The plot below shows the power vs sample size for this trial setup. 
...
From the above plot, we see that for a power of 0.9, we require $\approx$ 84 patients in each group. This gives us a total sample size of 168.
...
for (i in 1:1000){
  rejected <- F
  controldata <- rnorm(100, 10, sd =10)
  treatmentdata <- rnorm(100, 10, sd =10)
