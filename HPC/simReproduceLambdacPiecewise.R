...
for (i in 1:length(ScenarioList)){
  ...
  results <- foreach(j = 1:paramsList$NSims, .packages = c("survival", "dplyr", "rjags")) %dopar% {
    ...
    dataCombined <- generateDataPiecewise(ScenarioList[[i]]$lambdac, ScenarioList[[i]]$HR1, ScenarioList[[i]]$HR2,
                                 ScenarioList[[i]]$T1, ScenarioList[[i]]$T2, paramsList$numPatients, ScenarioList[[i]]$recTime)
