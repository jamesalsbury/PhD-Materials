...
overallFinalData <- data.frame(time = c(rexp(nPatients/2, rate = lambdac), ifelse(u>CP, (-log(u))/lambdac, (1/lambdat)*(bigT*lambdat-log(u)-bigT*lambdac))), group = c(rep("Control", nPatients/2),
  ...
  WieandOHR1 <- censorFunc(nEvents*0.5)$deltad
  WieandOHR2 <- censorFunc(nEvents*0.75)$deltad
  FinalOHR <- censorFunc(nEvents)$deltad
  ...
  OBFOHR1 <- censorFunc(nEvents*(1/3))$deltad
  OBFOHR2 <- censorFunc(nEvents*(2/3))$deltad
  ...
  PropOHR1 <- censorFunc(nEventsIA1)$deltad
  PropOHR2 <- censorFunc(nEventsIA2)$deltad
  
  #What would the outcome have been if we had not stopped for futility?
  if (censorFunc(nEvents)$test$chisq > qchisq(0.95, 1) & censorFunc(nEvents)$deltad<1){
        FinalOutcome <- "Successful trial"
      } else {
        FinalOutcome <- "Not a successful trial"
      }
...
if (k==1){
  outcomeDF <- data.frame(WieandOutcome = WieandOutcome, OBFOutcome = OBFOutcome, PropOutcome = PropOutcome, WBPP1 = WBPP1, WBPP2 = WBPP2,
           ...
           nEventsIA1 = nEventsIA1, nEventsIA2 = nEventsIA2, WieandOHR1 = WieandOHR1, WieandOHR2 = WieandOHR2,
           OBFOHR1 = OBFOHR1, OBFOHR2 = OBFOHR2, PropOHR1 = PropOHR1, PropOHR2 = PropOHR2, FinalOHR = FinalOHR, FinalOutcome = FinalOutcome)
...
HRVec <- seq(0.5, 1, by=0.05)

for (i in 1:length(HRVec)){
  assign(paste0("HR", HRVec[i]), readRDS(paste0("Data/OriginalData/HR", HRVec[i], ".rds")))
}
...
for (i in 1:length(HRVec)){
 p1 <- ggplot(get(paste0("HR", HRVec[i], "Look1")), aes(x = Outcome, y = BPP, color = Rule)) + geom_boxplot() + ylim(0,1) + 
   ggtitle(paste0("HR = ", HRVec[i])) + geom_hline(yintercept = 0.65) + xlab("Outcome at first IA")
 print(p1)
}

for (i in 1:length(HRVec)){
 p1 <- ggplot(get(paste0("HR", HRVec[i], "Look2")), aes(x = Outcome, y = BPP, color = Rule)) + geom_boxplot() + ylim(0,1) + 
   ggtitle(paste0("HR = ", HRVec[i])) + geom_hline(yintercept = 0.65) + xlab("Outcome at second IA")
 print(p1)
}
...
# HR0.5Look2$Outcome <- factor(HR0.5Look2$Outcome, levels = c("Already stopped", "Stop for futility at IA2", "Continue"), ordered = T)
# HR0.55Look2$Outcome <- factor(HR0.55Look2$Outcome, levels = c("Already stopped", "Stop for futility at IA2", "Continue"), ordered = T)
# HR0.6Look2$Outcome <- factor(HR0.6Look2$Outcome, levels = c("Already stopped", "Stop for futility at IA2", "Continue"), ordered = T)
# HR0.65Look2$Outcome <- factor(HR0.65Look2$Outcome, levels = c("Already stopped", "Stop for futility at IA2", "Continue"), ordered = T)
# HR0.7Look2$Outcome <- factor(HR0.7Look2$Outcome, levels = c("Already stopped", "Stop for futility at IA2", "Continue"), ordered = T)
# HR0.75Look2$Outcome <- factor(HR0.75Look2$Outcome, levels = c("Already stopped", "Stop for futility at IA2", "Continue"), ordered = T)
# HR0.8Look2$Outcome <- factor(HR0.8Look2$Outcome, levels = c("Already stopped", "Stop for futility at IA2", "Continue"), ordered = T)
# HR0.85Look2$Outcome <- factor(HR0.85Look2$Outcome, levels = c("Already stopped", "Stop for futility at IA2", "Continue"), ordered = T)
# HR0.9Look2$Outcome <- factor(HR0.9Look2$Outcome, levels = c("Already stopped", "Stop for futility at IA2", "Continue"), ordered = T)
# HR0.95Look2$Outcome <- factor(HR0.95Look2$Outcome, levels = c("Already stopped", "Stop for futility at IA2", "Continue"), ordered = T)
# HR1Look2$Outcome <- factor(HR1Look2$Outcome, levels = c("Already stopped", "Stop for futility at IA2", "Continue"), ordered = T)



# HR0.5$PropOutcome <- factor(HR0.5$PropOutcome, levels = c("Stop for futility at IA1", "Stop for futility at IA2", "Not a successful trial (at the end)", "Successful trial (at the end)"), ordered = T)
# 
# HR0.55$PropOutcome <- factor(HR0.55$PropOutcome, levels = c("Stop for futility at IA1", "Stop for futility at IA2", "Not a successful trial (at the end)", "Successful trial (at the end)"), ordered = T)
# 
# HR0.6$PropOutcome <- factor(HR0.6$PropOutcome, levels = c("Stop for futility at IA1", "Stop for futility at IA2", "Not a successful trial (at the end)", "Successful trial (at the end)"), ordered = T)
# 
# HR0.65$PropOutcome <- factor(HR0.65$PropOutcome, levels = c("Stop for futility at IA1", "Stop for futility at IA2", "Not a successful trial (at the end)", "Successful trial (at the end)"), ordered = T)
# 
# HR0.7$PropOutcome <- factor(HR0.7$PropOutcome, levels = c("Stop for futility at IA1", "Stop for futility at IA2", "Not a successful trial (at the end)", "Successful trial (at the end)"), ordered = T)
# 
# HR0.75$PropOutcome <- factor(HR0.75$PropOutcome, levels = c("Stop for futility at IA1", "Stop for futility at IA2", "Not a successful trial (at the end)", "Successful trial (at the end)"), ordered = T)
# 
# HR0.8$PropOutcome <- factor(HR0.8$PropOutcome, levels = c("Stop for futility at IA1", "Stop for futility at IA2", "Not a successful trial (at the end)", "Successful trial (at the end)"), ordered = T)
# 
# HR0.85$PropOutcome <- factor(HR0.85$PropOutcome, levels = c("Stop for futility at IA1", "Stop for futility at IA2", "Not a successful trial (at the end)", "Successful trial (at the end)"), ordered = T)
# 
# HR0.9$PropOutcome <- factor(HR0.9$PropOutcome, levels = c("Stop for futility at IA1", "Stop for futility at IA2", "Not a successful trial (at the end)", "Successful trial (at the end)"), ordered = T)
# 
# HR0.95$PropOutcome <- factor(HR0.95$PropOutcome, levels = c("Stop for futility at IA1", "Stop for futility at IA2", "Not a successful trial (at the end)", "Successful trial (at the end)"), ordered = T)
# 
# HR1$PropOutcome <- factor(HR1$PropOutcome, levels = c("Stop for futility at IA1", "Stop for futility at IA2", "Not a successful trial (at the end)", "Successful trial (at the end)"), ordered = T)

WieandDecisionFinal <- rep(NA, 100)

for (i in 1:100){
  if (HR0.5$WieandOutcome[i]=="Successful trial (at the end)"){
    WieandDecisionFinal[i] <- "Successful"
  } else if (HR0.5$PropOutcome[i]=="Not a successful trial (at the end)"){
    WieandDecisionFinal[i] <- "Unsuccessful"
  } else {
    WieandDecisionFinal[i] <- "Already stopped"
  }
}

HR1 <- cbind(HR1, PropDecisionIA2)

# saveRDS(HR0.5, file = "Data/HR0.5.rds")
# saveRDS(HR0.55, file = "Data/HR0.55.rds")
# saveRDS(HR0.6, file = "Data/HR0.6.rds")
# saveRDS(HR0.65, file = "Data/HR0.65.rds")
# saveRDS(HR0.7, file = "Data/HR0.7.rds")
# saveRDS(HR0.75, file = "Data/HR0.75.rds")
# saveRDS(HR0.8, file = "Data/HR0.8.rds")
# saveRDS(HR0.85, file = "Data/HR0.85.rds")
# saveRDS(HR0.9, file = "Data/HR0.9.rds")
# saveRDS(HR0.95, file = "Data/HR0.95.rds")
# saveRDS(HR1, file = "Data/HR1.rds")

HR0.5Look3 <- data.frame(Outcome = c(HR0.5$WieandDecisionIA2, HR0.5$OBFDecisionIA2, HR0.5$PropDecisionIA2), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR0.5$WBPP2, HR0.5$OBFBPP2, HR0.5$PropBPP2))
HR0.55Look3 <- data.frame(Outcome = c(HR0.55$WieandDecisionIA2, HR0.55$OBFDecisionIA2, HR0.55$PropDecisionIA2), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR0.55$WBPP2, HR0.55$OBFBPP2, HR0.55$PropBPP2))
HR0.6Look3 <- data.frame(Outcome = c(HR0.6$WieandDecisionIA2, HR0.6$OBFDecisionIA2, HR0.6$PropDecisionIA2), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR0.6$WBPP2, HR0.6$OBFBPP2, HR0.6$PropBPP2))
HR0.65Look3 <- data.frame(Outcome = c(HR0.65$WieandDecisionIA2, HR0.65$OBFDecisionIA2, HR0.65$PropDecisionIA2), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR0.65$WBPP2, HR0.65$OBFBPP2, HR0.65$PropBPP2))
HR0.7Look3 <- data.frame(Outcome = c(HR0.7$WieandDecisionIA2, HR0.7$OBFDecisionIA2, HR0.7$PropDecisionIA2), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR0.7$WBPP2, HR0.7$OBFBPP2, HR0.7$PropBPP2))
HR0.75Look3 <- data.frame(Outcome = c(HR0.75$WieandDecisionIA2, HR0.75$OBFDecisionIA2, HR0.75$PropDecisionIA2), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR0.75$WBPP2, HR0.75$OBFBPP2, HR0.75$PropBPP2))
HR0.8Look3 <- data.frame(Outcome = c(HR0.8$WieandDecisionIA2, HR0.8$OBFDecisionIA2, HR0.8$PropDecisionIA2), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR0.8$WBPP2, HR0.8$OBFBPP2, HR0.8$PropBPP2))
HR0.85Look3 <- data.frame(Outcome = c(HR0.85$WieandDecisionIA2, HR0.85$OBFDecisionIA2, HR0.85$PropDecisionIA2), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR0.85$WBPP2, HR0.85$OBFBPP2, HR0.85$PropBPP2))
HR0.9Look3 <- data.frame(Outcome = c(HR0.9$WieandDecisionIA2, HR0.9$OBFDecisionIA2, HR0.9$PropDecisionIA2), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR0.9$WBPP2, HR0.9$OBFBPP2, HR0.9$PropBPP2))
HR0.95Look3 <- data.frame(Outcome = c(HR0.95$WieandDecisionIA2, HR0.95$OBFDecisionIA2, HR0.95$PropDecisionIA2), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR0.95$WBPP2, HR0.95$OBFBPP2, HR0.95$PropBPP2))
HR1Look3 <- data.frame(Outcome = c(HR1$WieandDecisionIA2, HR1$OBFDecisionIA2, HR1$PropDecisionIA2), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR1$WBPP2, HR1$OBFBPP2, HR1$PropBPP2))

# saveRDS(HR0.5Look2, file = "Data/HR0.5Look2.rds")
# saveRDS(HR0.55Look2, file = "Data/HR0.55Look2.rds")
# saveRDS(HR0.6Look2, file = "Data/HR0.6Look2.rds")
# saveRDS(HR0.65Look2, file = "Data/HR0.65Look2.rds")
# saveRDS(HR0.7Look2, file = "Data/HR0.7Look2.rds")
# saveRDS(HR0.75Look2, file = "Data/HR0.75Look2.rds")
# saveRDS(HR0.8Look2, file = "Data/HR0.8Look2.rds")
# saveRDS(HR0.85Look2, file = "Data/HR0.85Look2.rds")
# saveRDS(HR0.9Look2, file = "Data/HR0.9Look2.rds")
# saveRDS(HR0.95Look2, file = "Data/HR0.95Look2.rds")
# saveRDS(HR1Look2, file = "Data/HR1Look2.rds")


#Do this for Look 1
p <- list()
for (i in 1:length(HRVec)){
 p[[i]] <- ggplot(get(paste0("HR", HRVec[i], "Look1")), aes(x = Outcome, y = BPP, color = Rule)) + geom_boxplot() + ylim(0,1) + 
   ggtitle(paste0("HR = ", HRVec[i])) + geom_hline(yintercept = 0.65) + xlab("Outcome at first IA")
}


png("Look1.png", units="in", width=32, height=15, res=700)
...
p <- list()
for (i in 1:length(HRVec)){
 p[[i]] <- ggplot(get(paste0("HR", HRVec[i], "Look2")), aes(x = Outcome, y = BPP, color = Rule)) + geom_boxplot() + ylim(0,1) + 
   ggtitle(paste0("HR = ", HRVec[i])) + geom_hline(yintercept = 0.65) + xlab("Outcome at second IA")
}


png("Look2.png", units="in", width=32, height=15, res=700)
do.call(grid.arrange, p)
dev.off()
...
for (i in 1:length(HRVec)){
  assign(paste0("HR", HRVec[i]), readRDS(paste0("Data/HROriginal/HR", HRVec[i], ".rds")))
}

for (i in 1:length(HRVec)){
  assign(paste0("HR", HRVec[i], "Look1"), readRDS(paste0("Data/HRLook1Data/HR", HRVec[i], "Look1.rds")))
}
...
# PropDecisionIA2 <- rep(NA, 100)
# 
# for (i in 1:100){
#   if (HR1.1$PropOutcome[i]=="Stop for futility at IA1"){
#     PropDecisionIA2[i] <- "Already stopped"
#   } else if (HR1.1$PropOutcome[i]=="Stop for futility at IA2"){
#     PropDecisionIA2[i] <- "Stop for futility at IA2"
#   } else {
#     PropDecisionIA2[i] <- "Continue"
#   }
# }
# 
# HR1.1 <- cbind(HR1.1, PropDecisionIA2)

getwd()


#HR0.7 = subset(HR0.7, select = -WieandDecisionIA2)

# saveRDS(HR0.5, file = "Data/HROriginal/HR0.5.rds")
# saveRDS(HR0.6, file = "Data/HROriginal/HR0.6.rds")
# saveRDS(HR0.7, file = "Data/HROriginal/HR0.7.rds")
# saveRDS(HR0.8, file = "Data/HROriginal/HR0.8.rds")
# saveRDS(HR0.9, file = "Data/HROriginal/HR0.9.rds")
# saveRDS(HR1, file = "Data/HROriginal/HR1.rds")
# saveRDS(HR1.1, file = "Data/HROriginal/HR1.1.rds")


# HR0.5Look1 <- data.frame(Outcome = c(HR0.5$WieandDecisionIA1, HR0.5$OBFDecisionIA1, HR0.5$PropDecisionIA1), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR0.5$WBPP1, HR0.5$OBFBPP1, HR0.5$PropBPP1), ObservedHR = c(HR0.5$WieandOHR1, HR0.5$OBFOHR1, HR0.5$PropOHR1))
# HR0.6Look1 <- data.frame(Outcome = c(HR0.6$WieandDecisionIA1, HR0.6$OBFDecisionIA1, HR0.6$PropDecisionIA1), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR0.6$WBPP1, HR0.6$OBFBPP1, HR0.6$PropBPP1), ObservedHR = c(HR0.6$WieandOHR1, HR0.6$OBFOHR1, HR0.6$PropOHR1))
# HR0.7Look1 <- data.frame(Outcome = c(HR0.7$WieandDecisionIA1, HR0.7$OBFDecisionIA1, HR0.7$PropDecisionIA1), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR0.7$WBPP1, HR0.7$OBFBPP1, HR0.7$PropBPP1), ObservedHR = c(HR0.7$WieandOHR1, HR0.7$OBFOHR1, HR0.7$PropOHR1))
# HR0.8Look1 <- data.frame(Outcome = c(HR0.8$WieandDecisionIA1, HR0.8$OBFDecisionIA1, HR0.8$PropDecisionIA1), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR0.8$WBPP1, HR0.8$OBFBPP1, HR0.8$PropBPP1), ObservedHR = c(HR0.8$WieandOHR1, HR0.8$OBFOHR1, HR0.8$PropOHR1))
# HR0.9Look1 <- data.frame(Outcome = c(HR0.9$WieandDecisionIA1, HR0.9$OBFDecisionIA1, HR0.9$PropDecisionIA1), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR0.9$WBPP1, HR0.9$OBFBPP1, HR0.9$PropBPP1), ObservedHR = c(HR0.9$WieandOHR1, HR0.9$OBFOHR1, HR0.9$PropOHR1))
# HR1Look1 <- data.frame(Outcome = c(HR1$WieandDecisionIA1, HR1$OBFDecisionIA1, HR1$PropDecisionIA1), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR1$WBPP1, HR1$OBFBPP1, HR1$PropBPP1), ObservedHR = c(HR1$WieandOHR1, HR1$OBFOHR1, HR1$PropOHR1))
# HR1.1Look1 <- data.frame(Outcome = c(HR1.1$WieandDecisionIA1, HR1.1$OBFDecisionIA1, HR1.1$PropDecisionIA1), Rule = c(rep("Wieand", 100), rep("OBF", 100), rep("Proposed", 100)), BPP = c(HR1.1$WBPP1, HR1.1$OBFBPP1, HR1.1$PropBPP1), ObservedHR = c(HR1.1$WieandOHR1, HR1.1$OBFOHR1, HR1.1$PropOHR1))

saveRDS(HR0.5Look1, file = "Data/HRLook1Data/HR0.5Look1.rds")
saveRDS(HR0.6Look1, file = "Data/HRLook1Data/HR0.6Look1.rds")
saveRDS(HR0.7Look1, file = "Data/HRLook1Data/HR0.7Look1.rds")
saveRDS(HR0.8Look1, file = "Data/HRLook1Data/HR0.8Look1.rds")
saveRDS(HR0.9Look1, file = "Data/HRLook1Data/HR0.9Look1.rds")
saveRDS(HR1Look1, file = "Data/HRLook1Data/HR1Look1.rds")
saveRDS(HR1.1Look1, file = "Data/HRLook1Data/HR1.1Look1.rds")
...
p <- list()
for (i in 1:length(HRVec)){
 p[[i]] <- ggplot(get(paste0("HR", HRVec[i], "Look1"))) + geom_point(aes(x = ObservedHR, y = BPP, color = Rule, shape = Outcome)) + ylim(0,1) + xlab("Observed HR at IA1") + xlim(0,2) + geom_vline(xintercept = 1, color = "red", lty = 2) + geom_hline(yintercept = 0.65, lty = 3, show.legend = T) +  ggtitle(paste0("HR = ", HRVec[i]))
}



png("HRLook1.png", units="in", width=32, height=15, res=700)
do.call(grid.arrange, p)
dev.off()
