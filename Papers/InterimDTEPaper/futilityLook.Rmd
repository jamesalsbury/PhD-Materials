...
plot(output)
 ...
 kmfit <- survfit(Surv(survival_time, status)~group, data = combinedData) 
  ...
  noCensor <- combinedData[combinedData$status==1,]
  
 censorkm <- survfit(Surv(survival_time, status)~group, data = noCensor) 
  
  plot(censorkm, col = c("blue", "red"), xlim=c(0,50)) 
...
# 
# for (i in 1:2){
#   trialTime <- seq(0, 50, by = 0.01)
# controlSurv <- exp(-lambda2output[i]*trialTime)
# lines(trialTime, controlSurv, col = "blue")
# treatmentSurv <- ifelse(trialTime < bigToutput[i], exp(-lambda2output[i]*trialTime),  exp(-lambda2output[i]*bigToutput[i]-lambda2output[i]*HRoutput[i]*(trialTime-bigToutput[i])))
# #treatmentSurv <- exp(-lambda2output[i]*bigToutput[i]-lambda2output[i]*HRoutput[i]*(trialTime-bigToutput[i]))
