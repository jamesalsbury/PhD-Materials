...
est <- stats4::mle(minuslogl = nll, start = list(bigTEst = 20,lambda2Est = 0.01, lambda1Est = 0.01), lower=  list(bigTEst=2, lambda2Est = 0.0001, lambda1Est = 0.0001), upper = list(bigTEst = 60))

#Showing the output of the mle() function
est


#Let us just look at the likelihood for T (we are assuming we know what lambda1 and lambda2 are here)

nllT <- function(bigTEst){
  j <- sum(treatmentData < bigTEst)
  loglikelihood <- (n+j)*log(lambda2)-lambda2*(sum(controlData)+sum(treatmentData[1:j]))+
  (m-j)*(log(lambda1)+lambda1*bigTEst-lambda2*bigTEst) - lambda1*sum(treatmentData[(j+1):m])
  #Returning the negative ll as this is what the mle() function expects
 return(-loglikelihood)
}
...
SA <- GenSA(par = c(15, 0.01, 0.01), fn = nlloptim, lower=c(1, 0.001, 0.001), upper=c(60, 10, 10))
