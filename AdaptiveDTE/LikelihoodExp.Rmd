...
combinedData$cens <- combinedData$time<IATime
...
model {
  ...
  for (i in 1:n){
    ...
    zeros.mean[i] <-  -l[i] + C
    l[i] <- ifelse(datCens[i]==1, log(lambdac)-lambdac*datTimes[i], -lambdac*datTimes[i])
  }
  for (i in (n+1):m){                                                                                                             
    zeros[i] ~ dpois(zeros.mean[i])
    zeros.mean[i] <-  -l[i] + C
    l[i] <- ifelse(datCens[i]==1, ifelse(datTimes[i]<bigT, log(lambdac)-lambdac*(datTimes[i]), log(lambdat)-lambdac*bigT-lambdat*(datTimes[i]-bigT)), 
...
model = jags.model(textConnection(modelstring), data = list(datTimes = combinedData$time, datCens = combinedData$cens, n= n, m=m))
