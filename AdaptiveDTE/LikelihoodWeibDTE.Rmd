...
nVec <- round(10^seq(0, 2, by=0.5))
IATimeVec <- seq(10, 60 ,by=10)

posteriorMatrix <-matrix(rep(list(), length(nVec)+length(IATimeVec)),nrow = length(nVec), ncol = length(IATimeVec))

for (i in 1:length(IATimeVec)){
  for (j in 1:length(nVec)){
    posteriorMatrix[j,i] <- list(TPPIA(nVec[j], IATimeVec[i]))
  }
}
...
for (i in 1:length(nVec)){
  for (j in 1:length(IATimeVec)){
    hist(unlist(posteriorMatrix[i,j])[2:1001])
  }
}
